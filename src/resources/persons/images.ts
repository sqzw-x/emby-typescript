...
export class Images extends APIResource {
  ...
  retrieve(index: number, params: ImageRetrieveParams, options?: RequestOptions): APIPromise<void> {
    ...
    return this._client.get(path`/Persons/${Name}/Images/${Type}/${index}`, {
      query,
      ...options,
   ...
   * Requires authentication as user
   */
  list(type: ImagesAPI.ImageType, params: ImageListParams, options?: RequestOptions): APIPromise<void> {
    const { Name, ...query } = params;
    return this._client.get(path`/Persons/${Name}/Images/${type}`, {
      query,
      ...options,
      headers: buildHeaders([{ Accept: '*/*' }, options?.headers]),
    });
