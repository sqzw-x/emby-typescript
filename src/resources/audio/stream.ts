...
export class Stream extends APIResource {
  ...
  retrieve(id: string, query: StreamRetrieveParams, options?: RequestOptions): APIPromise<void> {
    return this._client.get(path`/Audio/${id}/stream`, {
      query,
      ...options,
  ...
  retrieveWithContainer(
    container: string,
    params: StreamRetrieveWithContainerParams,
    options?: RequestOptions,
  ): APIPromise<void> {
    const { Id, ...query } = params;
    return this._client.get(path`/Audio/${Id}/stream.${container}`, {
      query,
      ...options,
      headers: buildHeaders([{ Accept: '*/*' }, options?.headers]),
    });
...
export interface StreamRetrieveWithContainerParams {
  /**
   * Path param: Item Id
   */
  Id: string;
