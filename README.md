...
The REST API documentation can be found on [emby.media](https://emby.media/community/index.php?/forum/47-developer-api). The full API of this library can be found in [api.md](api.md).
...
const response = await client.artists.list().asResponse();
console.log(response.headers.get('X-My-Header'));
console.log(response.statusText); // access the underlying Response object

const { data: queryResultBaseItem, response: raw } = await client.artists.list().withResponse();
...
1. Via the `EMBY_LOG` environment variable
